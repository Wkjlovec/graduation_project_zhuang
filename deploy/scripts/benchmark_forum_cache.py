...
def run(base_url: str, path: str, warm_runs: int, timeout_seconds: int, report_file: str | None) -> None:
    ...
    print("  缓存基准测试结果")
    print("=" * 50)
    print(f"  冷启动响应时间        {stats.cold_ms} ms")
    print(f"  热缓存平均响应时间    {stats.warm_avg_ms} ms")
    print(f"  热缓存最小响应时间    {stats.warm_min_ms} ms")
    print(f"  热缓存最大响应时间    {stats.warm_max_ms} ms")
    print(f"  热缓存 P95 响应时间   {stats.warm_p95_ms} ms")
    print(f"  性能提升百分比        {stats.improvement_percent}%")
    print(f"  热缓存请求次数        {stats.warm_runs}")
    print("=" * 50)
    print()
    if report_file:
